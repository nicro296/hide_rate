...
function check_load_iframe(iframe1){
    if (iframe1 != null 
        ...
        && iframe1.contentWindow.document.getElementById('app').children[1].children[0].children[0].children[0].children.length >= 5
        && iframe1.contentWindow.document.getElementById('app').children[1].children[0].children[0].children[0].children[4].children != null
        && iframe1.contentWindow.document.getElementById('app').children[1].children[0].children[0].children[0].children[4].children[0].children != null
        && iframe1.contentWindow.document.getElementById('app').children[1].children[0].children[0].children[0].children[4].children[0].children.length >= 2) {
        return true;
    }else{
        return false;
    }
...
function check_rate(target_rate){
    ...
    function jsLoaded() {
        ...
        if (check_load_iframe(iframe1)) {
            ...
            if(iframe1.contentWindow.document.getElementById('app').children[1].children[0].children[0].children[0].children[4].children[0].textContent.indexOf('レーティング')>-1){
                rate = iframe1.contentWindow.document.getElementById('app').children[1].children[0].children[0].children[0].children[4].children[0].children[1].textContent.substring(0,4).trim();
                if(rate >= target_rate){
...
function main(){
    chrome.storage.local.get(['bl_hide_rate_2Pick','bl_hide_opponent_rate_2Pick','target_rate_2Pick'],function(result){
                    ...
                    if(check_load()){
                        ...
                        if(div5.children!= null && div5.children.length >1){//対戦ページチェック
                            ...
                            if(result.target_rate_2Pick != null){
                                window.addEventListener("load", check_rate(result.target_rate_2Pick), false);
                            }else{
